...
void rqc_pke_decrypt(rbc_vec m, const rbc_qre u, const rbc_qre v, const uint8_t* sk) {
  ...
  hal_send_str("IN DEC HERE 1\n");
  rbc_field_init();
  hal_send_str("IN DEC HERE 2\n");
  rbc_qre_init_modulus(RQC_PARAM_N);
  hal_send_str("IN DEC HERE 3\n");
