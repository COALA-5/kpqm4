...
void random_init(AES_XOF_struct* o, int type) {
    if (type == 0) { //PRNG
        o = NULL;
    }
    else { //Seedexpander
        o = (AES_XOF_struct*)malloc(sizeof(AES_XOF_struct));
    }
}

/** 
* 240201 FROM Layered Rollo (2023)
...
void rbc_gabidulin_decode(rbc_vec m, const rbc_gabidulin gc, const rbc_qre y) {
  ...
  hal_send_str("IN GABIDULIN HERE 2\n");
  ...
  hal_send_str("IN GABIDULIN HERE 3\n");
  ...
  hal_send_str("IN GABIDULIN HERE 4\n");
  ...
  hal_send_str("IN GABIDULIN HERE 5\n");
  ...
  hal_send_str("IN GABIDULIN HERE 6\n");
  ...
  hal_send_str("IN GABIDULIN HERE 7\n");
