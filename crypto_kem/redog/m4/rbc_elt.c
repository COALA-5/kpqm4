...
int bsr(uint32_t x) {
  ...
  while (x != 0) {
    x >>= 1; // Right shift the number by 1
...
void rbc_elt_inv(rbc_elt o, const rbc_elt e) {
  ...
  while(u[0] != 1) {
    ...
    // 0 < j <= 64
      ...
      carry = v[0] >> (64 - j);
      u[0] ^= (uint64_t)(v[0] << j);
      u[1] ^= (uint64_t)(v[1] << j) ^ carry;

      carry = g[0] >> (64 - j);
      o[0] ^= (uint64_t)(g[0] << j);
      o[1] ^= (uint64_t)(g[1] << j) ^ carry;
    }

    // 64 < j < M
    else {
      u[1] ^= (uint64_t) (v[0] << (j - 64));
      o[1] ^= (uint64_t) (g[0] << (j - 64));
    }
  }
}
