...
void shake256_squeeze(uint8_t *out, size_t outlen, shake256incctx *state) {
  ...
  #endif
    state->ctx[25] =
        keccak_squeeze(out, outlen, state->ctx, state->ctx[25], SHAKE256_RATE);
  #ifdef PROFILE_HASHING
    uint64_t t1 = hal_get_time();
    hash_cycles += (t1-t0);
  #endif
...
//     uint64_t t0 = hal_get_time();
//   #endif
//     keccak_absorb_once(state->ctx, SHAKE128_RATE, in, inlen, 0x1F);
//     state->ctx[25] = SHAKE128_RATE;
//   #ifdef PROFILE_HASHING
//     uint64_t t1 = hal_get_time();
//     hash_cycles += (t1-t0);
//   #endif
...
//     uint64_t t0 = hal_get_time();
//   #endif
//     keccak_absorb_once(state->ctx, SHAKE256_RATE, in, inlen, 0x1F);
//     state->ctx[25] = SHAKE128_RATE;
//   #ifdef PROFILE_HASHING
//     uint64_t t1 = hal_get_time();
//     hash_cycles += (t1-t0);
