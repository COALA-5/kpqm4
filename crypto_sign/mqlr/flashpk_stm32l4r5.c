...
__attribute__((__section__(".flash_pk"))) const unsigned char __ba[BA_SIZE];
...
uint32_t flashpk_program_2words(uint32_t address, uint64_t data )
{
	...
	flash_program_double_word((&__ba[0])+address,data);
	flash_wait_for_last_operation();
...
uint32_t flashpk_program_n_2words(uint32_t address, const uint32_t *data , unsigned n )
{
	...
	while(n) {
		...
		flash_program_double_word( (&__ba[0])+address , dd );
		flash_wait_for_last_operation();
		address += 8;
		data += 2;
		n -= 1;
