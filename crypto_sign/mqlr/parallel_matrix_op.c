...
#define ADDR_FLASH_BA ((uint32_t)0x081ad000)
...
void UpperTrianglize_circ(unsigned char* btriC, const unsigned char* bcirc, unsigned Awidth /* _V */, unsigned size_batch /* _O */) 
{
	// flash memory setting
	uint32_t ba_start_addr = ADDR_FLASH_BA;

	hal_send_str("UpperTrianglize_circ here 1 (before malloc)");
	//unsigned char* bA = (unsigned char*)malloc(_O * _V * _V);
	unsigned char bA[_O] = {0,};
	...
	for (unsigned i = 0; i < _V; i++) {
		for (unsigned j = 0; j < _V; j++) {
			flashpk_program_n_2words(ba_start_addr + i * _O * _V + j * _O, bcirc2 + i + j * 2 * _V, size_batch);
			//memcpy(bA + i * _O * _V + j * _O, bcirc2 + i + j * 2 * _V, size_batch);
		}
	}
	printbytes(&ba_start_addr, size_batch);
	...
	for (unsigned i = 0; i < Aheight; i++) {
		for (unsigned j = 0; j < i; j++) {
			unsigned idx = idx_of_trimat(j, i, Aheight);
			gf256v_add(btriC + idx * size_batch, ba_start_addr + size_batch * (i * Awidth + j), size_batch);
		}
		gf256v_add(runningC, ba_start_addr + size_batch * (i * Awidth + i), size_batch * (Aheight - i));
		runningC += size_batch * (Aheight - i);
	}
